...
const TablaProductos = () => {
  ...
  const handleAddProduct = async (product) => {
    try {
      const productWithImage = {
        ...
        const response = await fetch((`${process.env.REACT_APP_API_URL}/products/agregar`), {
        method: 'POST',
        headers: {
  ...
  const handleConfirmEdit = async (updatedProduct) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/products/${updatedProduct.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
